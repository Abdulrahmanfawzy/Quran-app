...
if (search > 0) {
    ...
    .then((ress) => ress.json())
    .then((data2) => {
        console.log(data2); // reciter
        let {name} = data2.reciters[0];
        username.innerHTML = `القرأن الكريم بصوت الشيخ ${name} `;
        let { surah_list } = data2.reciters[0].moshaf[0];
        let arr = surah_list.split(",");
        let newArr = arr.sort(function(a, b) {
            return a - b;
        });
        for (let i = 0; i < newArr.length; i++) {
            fetch(`https://api.quran.com/api/v4/chapters/${newArr[i]}?language=ar`)
